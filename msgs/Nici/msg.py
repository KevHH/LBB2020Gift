...
def get_msg_list():
    ...
    msg_list.append( Message("Thanks,", "Adam and Elena,", 1.5) )
    msg_list.append( Message("for making", "the start of", 1.5) )
    msg_list.append( Message("our PhDs", "so fun!", 1.5) )
    msg_list.append( Message(" - Nici ", "(and Roberta)", 1.5))

    # or you can create "animations" like this
    for i in range(8):
        msg_list.append(Message( " " * i + "  \_/  ", " " * i + chr(169) + "(* *)" + chr(170), 1)) # iteratively add messages with number of spaces before ^_^ = i
    ########################
    ## CUSTOMISATION END #
    ########################
    return msg_list
