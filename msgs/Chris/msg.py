...
def get_msg_list():
    ...
    msg_list.append( Message("Thank you both", "for leading such", 1) )
    msg_list.append( Message("a wonderful", " bootcamp!", 1) )

    # or you can create "animations" like this
    for i in range(7):
        msg_list.append(Message( " " * i + "~~ :-) ~~", "     Chris", 0.1)) # iteratively add messages with number of spaces before ^_^ = i
    ########################
    ## CUSTOMISATION END #
    ########################
    return msg_list
